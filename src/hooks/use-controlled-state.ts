...
export function useControlledState<T, C = T>(
  ...
  let [stateValue, setStateValue] = useState<T>(value || defaultValue)
  ...
  let setValue = useCallback<(value: T) => void>(
    (value, ...args) => {
      let onChangeCaller: (value: any) => void = (value, ...onChangeArgs) => {
        if (onChange) {
      ...
      if (typeof value === 'function') {
        ...
        let updateFunction: (value: T) => T = (oldValue, ...functionArgs) => {
